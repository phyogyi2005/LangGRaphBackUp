...
if not API_KEYS:
    ...
    if os.environ.get("GOOGLE_API_KEY"):
        ...
        # We don't exit here to allow the server to start, but it will fail on generation
...
class RotationManager:
    ...
    def get_response(self, prompt):
        ...
        while True:
            # 1. CHECK IF ALL KEYS BLOCKED (RESET LOGIC)
            if len(self.active_keys) == 0:
                print("🔄 CRITICAL: All keys blocked. Restoring ALL keys and retrying...")
                self.active_keys = self.master_keys.copy()
                time.sleep(2) 

            current_key = self.active_keys[0]
            current_models_list = MASTER_MODELS.copy()
            key_failed_completely = True 

            # 2. INNER LOOP (MODELS)
            while len(current_models_list) > 0:
                current_model = current_models_list[0]
                ...
                try:
                    ...
                    return llm.invoke(prompt) # SUCCESS

                except Exception as e:
                    error_msg = str(e).lower()
                    print(f"⚠️ Error on Key ...{current_key[-4:]} | {current_model}: {error_msg[:30]}...")
                    
                    # LOGIC: Remove this model, try next model
                    current_models_list.pop(0)
            
            # 3. KEY BLOCKING LOGIC
            if key_failed_completely:
                print(f"⛔ Key ...{current_key[-4:]} failed with ALL models. BLOCKING KEY.")
                self.active_keys.pop(0)
                print(f"   📉 Remaining Active Keys: {len(self.active_keys)}")
...
def processing_node(state: ChatState):
    ...
    history_text = "\n".join(state["history"][-4:] if state["history"] else [])
    ...
    try:
        ...
        state["answer"] = f"System Busy: {str(e)}"
